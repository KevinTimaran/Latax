...
\begin{frame}{Resultados: Distancias Calculadas}
    ...
    \begin{columns}[c]
        ...
        \begin{block}{Distancia a cada Sensor ($R_i$)}
            \centering
            \scriptsize 
            \begin{tabular}{@{}lcccc@{}}
                \toprule
                \textbf{Sensor} & \textbf{$x_i$} & \textbf{$y_i$} & \textbf{$z_i$} & \textbf{$R_i$} \\ \midrule
        ...
        \begin{block}{Valores Críticos}
            \scriptsize
            \begin{itemize}
                \item \textbf{Mínimo:} S9 ($R_9 \approx 1.84$) \\
                \vspace{0.1cm}
                $\rightarrow$ \textit{Mayor amplitud esperada.}
                
                \vspace{0.4cm}
                
                \item \textbf{Máximo:} S8 ($R_8 \approx 6.12$) \\
                \vspace{0.1cm}
